...
class Simplibills extends Widget
{
    ...
    public function isAuthorized()
    {
        ...
        $query = "SELECT * 
                  ...
                  WHERE processname = 'Arbeitsnachweise'";
    ...
    public function getBearbeitung()
    {
        ...
        $query = "WITH RankedRows AS (
                ...
                LEFT JOIN JRINCIDENTS j ON h.step_id = j.process_step_id AND j.processname = 'RECHNUNGSBEARBEITUNG' AND (j.STATUS = 0 OR j.STATUS = 1) //TODO change the processname based on config
                INNER JOIN JRINCIDENT i ON j.processid = i.processid AND i.`status`= 0
                WHERE documentrevision_id = MaxRevisionID AND h.FAELLIGKEIT < CURDATE() //TODO change h.FAELLIGKEIT based on config 
                AND r.STATUS = 'Bearbeitung'
                GROUP BY h.STEP;";
        $result = $JobDB->query($query);

        $bearbeitung = array_fill(0, 8, 0);
        $stepMapping = ["1" => 0, "2" => 1, "3" => 2, "4" => 3, "7" => 3, "5" => 4, "15" => 5, "17" => 6, "30" => 7]; //TODO map the steps based on config
